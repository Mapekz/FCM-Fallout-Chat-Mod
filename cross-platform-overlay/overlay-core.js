...
// Candidate paths for FO76's Proton prefix user.reg, in priority order.
// $STEAM_COMPAT_DATA_PATH (set by Steam when launching a game directly) wins
// when present; otherwise try the two common Steam library locations.
function fo76UserRegCandidates(homeDir, appid = FO76_APPID, env = {}) {
  // eslint-disable-next-line global-require
  const join = require('path').join;
  const candidates = [];
  if (env.STEAM_COMPAT_DATA_PATH) {
    candidates.push(join(env.STEAM_COMPAT_DATA_PATH, 'pfx', 'user.reg'));
  }
