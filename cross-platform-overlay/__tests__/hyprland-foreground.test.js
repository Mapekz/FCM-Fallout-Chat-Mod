...
describe('parseHyprctlClientsForGame', () => {
  it('returns {x,y} from the matching client\'s at field', () => {
    const json = JSON.stringify([
      { class: 'kitty', at: [0, 0], address: '0xaaa' },
      { class: 'steam_app_1151340', at: [1920, 108], address: '0xbbb' },
    ]);
    expect(parseHyprctlClientsForGame(json, GAME_PATTERN)).toEqual({ x: 1920, y: 108 });
  });

  it('returns null when no client matches the class pattern', () => {
    const json = JSON.stringify([{ class: 'kitty', at: [0, 0] }]);
    expect(parseHyprctlClientsForGame(json, GAME_PATTERN)).toBeNull();
  });

  it('returns null on malformed JSON (must not throw)', () => {
    expect(() => parseHyprctlClientsForGame('not json{{{', GAME_PATTERN)).not.toThrow();
    expect(parseHyprctlClientsForGame('not json{{{', GAME_PATTERN)).toBeNull();
  });

  it('returns null on an empty clients array', () => {
    expect(parseHyprctlClientsForGame('[]', GAME_PATTERN)).toBeNull();
  });

  it('returns null when the matching client has a missing/malformed at', () => {
    expect(parseHyprctlClientsForGame(
      JSON.stringify([{ class: 'steam_app_1151340' }]), GAME_PATTERN,
    )).toBeNull();
    expect(parseHyprctlClientsForGame(
      JSON.stringify([{ class: 'steam_app_1151340', at: [1920] }]), GAME_PATTERN,
    )).toBeNull();
    expect(parseHyprctlClientsForGame(
      JSON.stringify([{ class: 'steam_app_1151340', at: '1920,108' }]), GAME_PATTERN,
    )).toBeNull();
  });
});

describe('parseHyprctlClientsForAddress', () => {
  it('returns the matching client\'s address string', () => {
    const json = JSON.stringify([
      { class: 'kitty', at: [0, 0], address: '0xaaa' },
      { class: 'fallout-chat-mod', at: [100, 200], address: '0x61e0a0c0e080' },
    ]);
    expect(parseHyprctlClientsForAddress(json, OVERLAY_PATTERN)).toBe('0x61e0a0c0e080');
  });

  it('returns null when no client matches the class pattern', () => {
    const json = JSON.stringify([{ class: 'kitty', address: '0xaaa' }]);
    expect(parseHyprctlClientsForAddress(json, OVERLAY_PATTERN)).toBeNull();
  });

  it('returns null on malformed JSON (must not throw)', () => {
    expect(() => parseHyprctlClientsForAddress('not json{{{', OVERLAY_PATTERN)).not.toThrow();
    expect(parseHyprctlClientsForAddress('not json{{{', OVERLAY_PATTERN)).toBeNull();
  });

  it('returns null on an empty clients array', () => {
    expect(parseHyprctlClientsForAddress('[]', OVERLAY_PATTERN)).toBeNull();
  });

  it('returns null when the matching client has a missing/malformed address', () => {
    expect(parseHyprctlClientsForAddress(
      JSON.stringify([{ class: 'fallout-chat-mod' }]), OVERLAY_PATTERN,
    )).toBeNull();
    expect(parseHyprctlClientsForAddress(
      JSON.stringify([{ class: 'fallout-chat-mod', address: 123 }]), OVERLAY_PATTERN,
    )).toBeNull();
    expect(parseHyprctlClientsForAddress(
      JSON.stringify([{ class: 'fallout-chat-mod', address: '' }]), OVERLAY_PATTERN,
    )).toBeNull();
  });
});
