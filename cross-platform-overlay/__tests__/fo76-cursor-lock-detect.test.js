...
describe('fo76UserRegCandidates', () => {
  ...
  it('STEAM_COMPAT_DATA_PATH, when present, is the first candidate', () => {
    const compat = '/compat/data';
    const got = fo76UserRegCandidates(home, FO76_APPID, { STEAM_COMPAT_DATA_PATH: compat });
    expect(got[0]).toBe(join(compat, 'pfx', 'user.reg'));
    expect(got).toContain(steam);
    expect(got).toContain(xdg);
