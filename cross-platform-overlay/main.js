...
function readFo76GrabSettings() {
  const candidates = overlayCore.fo76UserRegCandidates(os.homedir(), overlayCore.FO76_APPID, process.env);
  ...
  if (fgTool === 'hyprctl') {
    try {
      exec('hyprctl clients -j', { timeout: 2000, shell: '/bin/sh' }, (err, stdout) => {
        ...
        let pos = null;
        try { pos = overlayCore.parseHyprctlClientsForGame(stdout, pattern); } catch { pos = null; }
        if (!pos) { onProbeFail('probe-unparsed'); return; }
        applyGamePos(pos.x, pos.y);
      });
    } catch (e) {
...
function syncHyprlandPin(reason) {
  ...
  try {
    exec('hyprctl clients -j', { timeout: 2000, shell: '/bin/sh' }, (err, stdout) => {
      ...
      let address = null;
      try { address = overlayCore.parseHyprctlClientsForAddress(stdout, 'fallout-chat-mod'); } catch (e) {
...
app.on('before-quit', () => {
  ...
  } else if (IS_LINUX && IS_HYPRLAND) {
    // Force unpin even if the game is still running (mirrors the KDE path
    // always-removing the rule on quit rather than going through the gate).
    try { gameRunning = false; syncHyprlandPin('before-quit'); } catch { /* ignore */ }
  }
});
